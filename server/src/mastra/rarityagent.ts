...
import { domainDataTool } from "./domaindatatool"; 
...
export const analysisOutputSchema = z.object({
  ...
  rarityScore: z.number().int().min(10).max(100).describe("The final, composite rarity score for the domain, calculated as a weighted average of all other metrics (10-100)."),
  
  // The six detailed insight scores (10-100 scale)
  tld: z.number().int().min(10).max(100).describe("Score representing the Top-Level Domain's prestige or inherent scarcity (10-100). Premium TLDs score high."),
  trend: z.number().int().min(10).max(100).describe("Score reflecting the Google Trends interest (0-100 scale) for related keywords (10-100)."),
  age: z.number().int().min(10).max(100).describe("Score based on the domain's age in years, rewarding older, more established domains (10-100)."),
  mcap: z.number().int().min(10).max(100).describe("Score based on the onchain market capitalization of the collection/domain (10-100). Higher MCAP scores high."),
  volume: z.number().int().min(10).max(100).describe("Score derived from recent trading volume (10-100). High volume scores high."),
  sentiment: z.number().int().min(10).max(100).describe("Composite score representing market sentiment, calculated from buyout price, open onchain offers, and offeror's token balance (10-100)."),

  // A brief, objective analysis of the scores
  analysis: z.string().describe("A concise, objective analysis (two to three sentences) summarizing the key drivers behind the calculated scores. You MUST NOT give an investment recommendation (e.g., STRONG_BUY, STRONG_SELL, or HOLD)."),
});
...
export const rarityAgent = new Agent({
  name: "Domain Rarity Agent",
  instructions,
  model: google("gemini-2.5-flash"),
  tools: { domainDataTool }
});
